...
bool ESP8266_ConnectTCPServer(const char *ip, uint16_t port, uint8_t conn_id)
{
	...
	if (conn_id == 0)
	{
		/* conn_id = 0: single connection mode */
		snprintf(cmd, sizeof(cmd), "AT+CIPSTART=\"TCP\",\"%s\",%d", ip, port);
	}
	else
	{
		/* conn_id > 0: multi-connection mode */
...
bool ESP8266_SetTransparentMode(TransparentModeTypeDef mode)
{
	...
	cmd = (mode == TRANSPARENT_OFF) ? "AT+CIPMODE=0" : "AT+CIPMODE=1";
...
bool ESP8266_SetSendMode(void)
{
	...
	if (ESP8266_Cmd("AT+CIPSEND") == false)
	{
		return false;
	}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	...
	else if (huart->Instance == USART3)
	{
		...
		if (ESP8266_TransparentMode == true)
		{
